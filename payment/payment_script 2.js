document.addEventListener("DOMContentLoaded", function () {
    ...
    const orders = document.querySelectorAll(`order_details`);
    ...
    const b_authors = document.querySelectorAll(`.tag_author`);
    const total_value = document.querySelector(".total_value");
    let amount = b_authors.length;
    ...
    const assign_values = function () {
      ...
      for (let i = 0; i < amount; i++) {
        ...
        b_prices[i].textContent = `${data[i * 9 + 2].textContent}$`;
        b_authors[i].textContent= authors[i];
      }
    };
    assign_values();
    for (let i = 0; i < amount; i++) {
